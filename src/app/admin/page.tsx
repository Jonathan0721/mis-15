...
export default function AdminPage() {
  ...
  const handleDeletePost = async (postId: string) => {
    if (!confirm("�Seguro que deseas eliminar esta publicaci�n permanentemente?")) return;
    ...
    <main className="min-h-screen bg-slate-900 text-white pb-12">
      ...
      <div className="max-w-4xl mx-auto px-4 pt-6 space-y-6">
        ...
        <div className="bg-slate-800/40 border border-white/10 rounded-3xl p-4 sm:p-6">
          ...
          {posts.length === 0 ? (
            <p className="text-sm text-slate-500 py-8 text-center">A�n no se ha publicado ning�n recuerdo.</p>
          ) : (
                  ...
                  <div>
                    ...
                    <div className="p-3">
                      <div className="flex items-center justify-between text-xs mb-1">
                        ...
                        <span className="text-slate-500">?? {post.likes || 0}</span>
                      </div>
                      {post.message && (
